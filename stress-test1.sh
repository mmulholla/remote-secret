...
# the secrets. After all secrets are created the second loop checks the status in each
# remote secret.
...
sleeptime=$((NUM_NAMESPACES + 1 ))
sleeptime=$(($((sleeptime * NUM_SECRETS))+10))
echo -e "\n4. Wait for reconcile - sleep $(time_in_mins_and_seconds $sleeptime)."
echo -e "\twait started at $(date)"
sleep $sleeptime

FAILS=()
RETRIES=()
echo -e "\n5. Check remote secrets have recieved data"
for j in $(seq $NUM_SECRETS)
do
  for i in $(seq $NUM_NAMESPACES)
  do
    RS="remotesecret/$RS_NAME_PREFIX$i-$j"
    REASON=$(check_resource "$RS" "$NS_PREFIX$i")
    if [[ "$REASON" == "DataFound" ]]; then
        echo "PASS: $RS, Reason: $REASON"
    else
        echo "$RS: Unexpected reason: $REASON, will retry later!"
        RETRIES+=( $RS $NS_PREFIX$i )
    fi
  done
done
...
if [[ ${#RETRIES[@]} > 0 ]]; then
    retries=$((${#RETRIES[@]} / 2))
    # calculate sleep time loosely based on time previously waited and the
    # number of successes achieved during that wait.
    successes=$(($(($NUM_NAMESPACES * $NUM_SECRETS)) - $retries + 1))
    success_time=$(( $(($sleeptime / $successes)) + 2 ))
    sleeptime=$(( $(($retries * $success_time)) + $retries ))
    echo -e "\n5a. wait $(time_in_mins_and_seconds $sleeptime) then perform $retries retries."
    echo -e "\twait started at $(date)"
    sleep $sleeptime
    RS_NAME=""
    for retry in ${RETRIES[@]}; do
      if [[ -z $RS_NAME ]]; then
        ...
        NS=$retry
        echo "re-check resource: $RS_NAME"
        REASON=$(check_resource "$RS_NAME" "$NS")
        if [[ "$REASON" == "DataFound" ]]; then
            echo "PASS: $RS_NAME, Reason: $REASON"
        else
